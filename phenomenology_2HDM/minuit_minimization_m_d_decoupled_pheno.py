...
def least_squares(M_u_0_0_re, M_u_0_0_im, M_u_0_1_re, M_u_0_1_im, M_u_0_2_re, M_u_0_2_im, M_u_0_3_re, M_u_0_3_im,
    ...
    if index == 2:
        M_d = np.array([[0 + 0 * 1j,               0 + 0 * 1j, x1 + 0 * 1j],
                        [0 + 0 * 1j,              x2 + 0 * 1j,
                            rho * cmath.exp(theta * 1j) + 0 * 1j],
                        [MASS_DECOUPLED + 0 * 1j,   0 + 0 * 1j, 0 + 0 * 1j]])
    else:
        M_d = np.array([[0 + 0 * 1j,               0 + 0 * 1j, x1 + 0 * 1j],
                        [0 + 0 * 1j,  MASS_DECOUPLED + 0 *
                            1j, 0 + 0 * 1j],
                        [x2 + 0 * 1j, 0 + 0 * 1j, rho * cmath.exp(theta * 1j) + 0 * 1j]])
...
def run_Minuit(pair, par_values, control, position=None):
    ...
    m.fixed["index"] = True
    m.limits["x2"] = (MASS_M, MASS_P)
    m.limits["theta"] = (0, 2 * np.pi)
...
def info_minimum(par_values):
    ...
    string = f"m_u = {D_u[0] / MeV} MeV/ chi_square_ratio_ut = {chi_square_ratio_ut}\n"
    string += f"m_c = {D_u[1] / GeV} GeV/ chi_square_ratio_ct = {chi_square_ratio_ct}\n"
    string += f"m_t = {D_u[2] / GeV} GeV\n"
    string += f"m_VLQ = {D_u[3] / TeV} TeV / chi_square_m_VLQ = {chi_square_m_VLQ}\n"
    string += f"m_d = {D_d[0] / MeV} MeV / chi_square_ratio_db = {chi_square_ratio_db}\n"
    string += f"m_s = {D_d[1] / MeV} MeV/ chi_square_m_ratio_sb = {chi_square_ratio_sb}\n"
    string += f"m_b = {D_d[2] / GeV} GeV\n"
