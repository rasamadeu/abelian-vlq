...
# Number of columns for table with notation
...
def search_decomp_notation(decomposition, notation_list, option):
    ...
    if option == "u":
        Y_u_1, Y_u_2 = _2HDM.auxiliary_decomposition_matrix_form(
            decomposition[0], "u")
        _, fourth_row = fourth_row_notation(decomposition[1],
                                            decomposition[2])
    if option == "d":
        Y_d_1, Y_d_2 = _2HDM.auxiliary_decomposition_matrix_form(
            decomposition[3], "d")
...
def auxiliary_decomposition(set_maximally_restrictive_pairs, notation_list, option):
    ...
    while indices != []:
                ...
                for elem in set_maximally_restrictive_pairs[indices[j]][2]:
                    if option == "u":
                        decompositions.append(
                            [elem[index_decom], elem[1], elem[2]])
                    if option == "d":
        ...
        notation_decomp = search_texture_notation(
            set_maximally_restrictive_pairs[indices[i]][1][index], notation_list)
        notation_decomp = notation_decomp.split("_")

        for order, elem in enumerate(decompositions):
            elem_matrix_form_1, elem_matrix_form_2 = _2HDM.auxiliary_decomposition_matrix_form(
                elem[0], option)
    ...
    set_decompositions = sorted(
        set_decompositions, key=lambda x: int(x[0].split("^")[0]))
    ...
    with open(filename, option) as f:
        ...
        while i < len(notation_list[0]):
            while (j < 4
                ...
                table = table + \
                    f"        {string.punctuation[3]}{notation_list[0][i + j][0]} \\sim {texture_latex_format(notation_list[0][i + j][1])}{string.punctuation[3]} &\n"
                j += 1
        ...
        while i < len(notation_list[1]):
            while (j < 4
                ...
                table = table + \
                    f"        {string.punctuation[3]}{notation_list[1][i + j][0]} \\sim {texture_latex_format(notation_list[1][i + j][1])}{string.punctuation[3]} &\n"
                j += 1
        ...
        for i in range(50, 60):
            ...
            for pair_1 in set_pairs_1_sigma:
                if np.array_equal(notation_list[0][i][1], pair_1[1][0]):
                    down_1.append(search_texture_notation(
                        pair_1[1][1], notation_list[1]))

            if down_1 != []:
                down_1 = sorted(down_1, key=lambda x: int(
                    x.split("^")[0]) + int(x.split("^")[1][3:-1]))
        ...
        for i in range(15, 22):
            ...
            for pair_1 in set_pairs_1_sigma:
                if np.array_equal(notation_list[1][i][1], pair_1[1][1]):
                    up_1.append(search_texture_notation(
                        pair_1[1][0], notation_list[0]))

            if up_1 != []:
                up_1 = sorted(up_1, key=lambda x: int(
                    x.split("^")[0]) + int(x.split("^")[1][3:-1]))
        ...
        for i in range(50, 60):
            ...
            for pair_3 in set_pairs_3_sigma:
                if np.array_equal(notation_list[0][i][1], pair_3[1][0]):
                    down_3.append(search_texture_notation(
                        pair_3[1][1], notation_list[1]))

            if down_3 != []:
                down_3 = sorted(down_3, key=lambda x: int(
                    x.split("^")[0]) + int(x.split("^")[1][3:-1]))
        ...
        for i in range(15, 22):
            ...
            for pair_3 in set_pairs_3_sigma:
                if np.array_equal(notation_list[1][i][1], pair_3[1][1]):
                    up_3.append(search_texture_notation(
                        pair_3[1][0], notation_list[0]))

            if up_3 != []:
                up_3 = sorted(up_3, key=lambda x: int(
                    x.split("^")[0]) + int(x.split("^")[1][3:-1]))
        ...
        for i, info in enumerate(list_table_1):
            ...
            if not isinstance(info[0], list):
                line = line + \
                    f"{string.punctuation[3]}{info[0]}{string.punctuation[3]} "
            else:
                for notation_up in info[0]:
                    line = line + \
                        f"{string.punctuation[3]}{notation_up}{string.punctuation[3]} "
            line = line + " & "
            if not isinstance(info[1], list):
                line = line + \
                    f"{string.punctuation[3]}{info[1]}{string.punctuation[3]} "
            else:
                for notation_down in info[1]:
                    line = line + \
                        f"{string.punctuation[3]}{notation_down}{string.punctuation[3]} "
            line = line + "\\\\\n"
            f.write(line)
        ...
        for decomposition in notation_decomp[0]:
            f.write(
                f"        {string.punctuation[3]}{decomposition[0]}{string.punctuation[3]} &\n")
            f.write(
                f"        {string.punctuation[3]}{decomposition[4]}{string.punctuation[3]} &\n")
            f.write(
                f"        {string.punctuation[3]}{decomposition[5]}{string.punctuation[3]} &\n")
            f.write(
                f"        {string.punctuation[3]}{decomposition[6]}{string.punctuation[3]} ")
            if i < 1:
                f.write(f"&\n\n")
                i += 1
            else:
        ...
        for decomposition in notation_decomp[1]:
            f.write(
                f"        {string.punctuation[3]}{decomposition[0]}{string.punctuation[3]} &\n")
            f.write(
                f"        {string.punctuation[3]}{decomposition[3]}{string.punctuation[3]} &\n")
            f.write(
                f"        {string.punctuation[3]}{decomposition[4]}{string.punctuation[3]} ")
            if i < 2:
                f.write(f"&\n\n")
        ...
        for pair in set_decompositions_3_sigma:
            for decomposition in pair[2]:
                set_viable_decomp.append(get_decomposition_notation(
                    decomposition, notation_decomp))

        i = 0
        while i < len(set_viable_decomp):
            j = i + 1
            set_viable_decomp[i][0] = f"{string.punctuation[3]}" + \
                set_viable_decomp[i][0] + f"{string.punctuation[3]}"
            while j < len(set_viable_decomp):
                if set_viable_decomp[i][1] == set_viable_decomp[j][1]:
                    set_viable_decomp[i][0] = set_viable_decomp[i][0] + \
                        f" {string.punctuation[3]}{set_viable_decomp[j][0]}{string.punctuation[3]}"
                    set_viable_decomp.pop(j)
                else:
                    j += 1

            f.write(
                f"        {set_viable_decomp[i][0]} & {string.punctuation[3]}{set_viable_decomp[i][1]}{string.punctuation[3]}\\\\\n\n")
            i += 1
...
def main():
    ...
    set_mrt_min = io_mrt.read_mrt_after_min(args[0])
    set_mrt_sym = io_mrt.read_mrt_after_sym(args[0])

    # Define notation list
    notation_list = define_notation_list(set_mrt_min)

    write_table_notation(notation_list, TABLE_NOTATION)
    write_table_pairs(set_mrt_min, notation_list, TABLE_PAIRS)
