...
#    plt.show()
    ...
    ax.annotate(r'$5_3^d$', (x[0], y[0]),
                (x[0] + 2e-5, y[0] - 3e-5), color="red")
    # texture 5_1 / no pheno / d decoupled
    ax.annotate(r'$5_1^d$', (x[1], y[1]),
                (x[1] + 2e-5, y[1] - 3e-5), color="blue")
    # texture 5_3 / no pheno / s decoupled
    ax.annotate(r'$5_3^s$', (x[2], y[2]),
                (x[2] - 3e-5, y[2] - 3e-5), color="red")
    # texture 5_1 / no pheno / s decoupled
    ax.annotate(r'$5_1^s$', (x[3], y[3]),
                (x[3] + 2e-5, y[3] - 3e-5), color="blue")
    # texture 5_3 / no pheno / b decoupled
    ax.annotate(r'$5_3^b$', (x[4], y[4]),
                (x[4] + 2e-5, y[4] + 2e-5), color="red")
    # texture 5_1 / no pheno / b decoupled
    ax.annotate(r'$5_1^b$', (x[5], y[5]),
                (x[5] - 2e-5, y[5] + 3e-5), color="blue")
    # texture 5_3 / with pheno / d decoupled
    ax.annotate(r'$5_3^d$', (x[6], y[6]),
                (x[6] - 1e-5, y[6] - 3e-5), color="red")
    # texture 5_1 / with pheno / d decoupled
    ax.annotate(r'$5_1^d$', (x[7], y[7]),
                (x[7] + 1e-5, y[7] - 5e-5), color="blue")
    # texture 5_3 / with pheno / s decoupled
    ax.annotate(r'$5_3^s$', (x[8], y[8]),
                (x[8] + 1e-5, y[8] + 3e-5), color="red")
    # texture 5_1 / with pheno / s decoupled
    ax.annotate(r'$5_1^s$', (x[9], y[9]),
                (x[9] - 2e-5, y[9] - 3e-5), color="blue")
    # texture 5_3 / with pheno / b decoupled
    ax.annotate(r'$5_3^b$', (x[10], y[10]),
                (x[10] + 2e-5, y[10] - 3e-5), color="red")
    # texture 5_1 / with pheno / b decoupled
    ax.annotate(r'$5_1^b$', (x[11], y[11]),
                (x[11] - 1e-5, y[11] + 3e-5), color="blue")
    ax.annotate("SM", (0.2250, 0.97435), (0.2250 - 7e-5, 0.97435 - 5e-5),
                color="darkviolet")  # texture 5_1 / with pheno / b decoupled

    # Subplot 1
    w = (0.97445-0.9736)
    h = (0.97445-0.9736)
    subaxis_a = ax.inset_axes(
        [(0.22405-0.2240)/h, (0.9739-0.9736)/h, (0.2245-0.2242)/w, (0.9742-0.9739)/h])
    subaxis_a.tick_params(axis='both', labelsize=10)
    ...
    subaxis_a.annotate(r'$5_1^d$', (x[1], y[1]),
                       (x[1] + 0.5e-5, y[1] - 1e-5), color="blue")
    # texture 5_1 / no pheno / d decoupled
    subaxis_a.annotate(r'$5_3^s$', (x[2], y[2]),
                       (x[2] - 0.75e-5, y[2] - 1e-5), color="red")
    # texture 5_1 / no pheno / d decoupled
    subaxis_a.annotate(r'$5_3^b$', (x[4], y[4]),
                       (x[4] + 0.5e-5, y[4] + 0.5e-5), color="red")
    ...
    subaxis_a.tick_params(axis='x', labelsize=8)
    subaxis_a.tick_params(axis='y', labelsize=8)
    subaxis_a.xaxis.get_major_formatter().set_useOffset(False)
    subaxis_a.xaxis.set_major_locator(ticker.FixedLocator([0.22416, 0.22422]))
    subaxis_a.yaxis.set_major_locator(ticker.FixedLocator([0.97366, 0.97376]))
    ...
    subaxis_a.tick_params(which='both', labelbottom=False, labeltop=True, labelleft=False,
                          labelright=True, bottom=False, top=True, left=False, right=True)

    # Subplot 2
    w = (0.2254-0.224)
    h = (0.97445-0.9736)
    subaxis_b = ax.inset_axes(
        [(0.2248-0.2240)/w, (0.97365-0.9736)/h, (0.2253-0.2248)/w, (0.9740-0.97365)/h])
    subaxis_b.tick_params(axis='both', labelsize=10)
    ...
    # texture 5_1 / no pheno / b decoupled
    subaxis_b.annotate(
        r'$5_1^b$', (x[5], y[5]), (x[5] - 1.2e-5, y[5] - 0.5e-5), color="blue")
    # texture 5_3 / with pheno / d decoupled
    subaxis_b.annotate(r'$5_3^d$', (x[6], y[6]),
                       (x[6] - 1.2e-5, y[6] - 0.5e-5), color="red")
    # texture 5_1 / with pheno / s decoupled
    subaxis_b.annotate(
        r'$5_1^s$', (x[9], y[9]), (x[9] - 1e-5, y[9] - 0.5e-5), color="blue")
    # texture 5_3 / with pheno / b decoupled
    subaxis_b.annotate(
        r'$5_3^b$', (x[10], y[10]), (x[10] - 1.2e-5, y[10] - 0.5e-5), color="red")
    # texture 5_1 / with pheno / b decoupled
    subaxis_b.annotate(
        r'$5_1^b$', (x[11], y[11]), (x[11] + 0.35e-5, y[11] - 0.5e-5), color="blue")
    subaxis_b.annotate("SM", (0.2250, 0.97435), (0.2250 -
                       0.5e-5, 0.97435 - 1.25e-5), color="darkviolet")
    ...
    subaxis_b.xaxis.set_major_locator(ticker.FixedLocator([0.22495, 0.22510]))
    subaxis_b.yaxis.set_major_locator(ticker.FixedLocator([0.9743, 0.9744]))
