...
MASS_P = MASS_B
MASS_M = MASS_D
FILENAME = "2500_tries/s_decoupled_5_3_8"

N_POINTS = 100
X2 = np.array([4.6701 + i / (N_POINTS + 1) * (0.0002)
               for i in range(N_POINTS)]) * MeV
X2 = X2[70:80]
print(X2)
THETA = np.array([0.01 + i / (N_POINTS) * (1)
...
def info_minimum(par_values):
    ...
    D_u = abs(D_u * abs(MASS_T / D_u[2]))
    x = np.array([np.exp(-np.angle(V[0, 0]) * 1j),
                 np.exp(-np.angle(V[0, 1]) * 1j), 1. + 0 * 1j], dtype=np.complex128)
    V = V @ np.diag(x)
    F = V @ V.conj().T
    chi_square_K = pheno.K0K0bar(V, D_u[1], D_u[2], D_u[3], 0)
    chi_square_eps = pheno.eps_ratio(V, F, D_u[0], D_u[1], D_u[2], D_u[3], 0)
    chi_square_D = pheno.D0D0bar(F, 0)
    chi_square_Bd = pheno.BdBdbar(V, D_u[2], D_u[3], 0)
    chi_square_Bs = pheno.BsBsbar(V, D_u[2], D_u[3], 0)
    chi_square_Bd_decay = pheno.Bd_mumu_decay(
        V, F, D_u[0], D_u[1], D_u[2], D_u[3], 0)
    chi_square_Bs_decay = pheno.Bs_mumu_decay(
        V, F, D_u[0], D_u[1], D_u[2], D_u[3], 0)
    chi_square_K1_decay = pheno.K_decay_bound_1(
        V, F, D_u[0], D_u[1], D_u[2], D_u[3], 0)
    chi_square_K2_decay = pheno.K_decay_bound_2(
        V, F, D_u[0], D_u[1], D_u[2], D_u[3], 0)
    chi_square_K3_decay = pheno.K_decay_bound_3(
        V, F, D_u[0], D_u[1], D_u[2], D_u[3], 0)
    ...
    string += f"PHENO:\n"
    if chi_square_K:
        string += f"chi_square_K = {chi_square_K}\n"
    if chi_square_eps:
        string += f"chi_square_eps = {chi_square_eps}\n"
    if chi_square_D:
        string += f"chi_square_D = {chi_square_D}\n"
    if chi_square_Bd:
        string += f"chi_square_Bd = {chi_square_Bd}\n"
    if chi_square_Bs:
        string += f"chi_square_Bs = {chi_square_Bs}\n"
    if chi_square_Bd_decay:
        string += f"chi_square_Bd_decay = {chi_square_Bd_decay}\n"
    if chi_square_Bs_decay:
        string += f"chi_square_Bs_decay = {chi_square_Bs_decay}\n"
    if chi_square_K1_decay:
        string += f"chi_square_K1_decay = {chi_square_K1_decay}\n"
    if chi_square_K2_decay:
        string += f"chi_square_K2_decay = {chi_square_K2_decay}\n"
    if chi_square_K3_decay:
        string += f"chi_square_K3_decay = {chi_square_K3_decay}\n"
...
def print_restrictive_pairs_from_minuit(output,
    ...
    with open(f"{filename}", "w") as f:
       # f.write(
       #    f'{"x2" : >15} {"theta" : >15} {"chi^2" : >15} {"chi^2_gamma" : >15} {"delta" : >15} {"chi^2_delta" : >15} {"m_T(TeV)" : >15} {"chi^2_K" : >15} {"chi^2_K" : >15} {"chi^2_eps" : >15} {"chi^2_D" : >15} {"chi^2_Bd_re" : >15} {"chi^2_Bd_im" : >15} {"chi^2_Bs_re" : >15} {"chi^2_Bs_im" : >15} {"chi^2_Bd_decay" : >15} {"chi^2_Bs_decay" : >15} {"chi^2_K1_decay" : >15} {"chi^2_K2_decay" : >15} {"chi^2_K3_decay" : >15} {"chi^2_Vud" : >15} {"chi^2_Vus" : >15} {"chi^2_Vub" : >15}\n\n')
        # for i in range(N_POINTS ** 2):
        for i in range(N_POINTS * 10):
            f.write(
                f"{output[i][2] / MeV: 15.7f} {output[i][3]: 15.6f} {output[i][0]: 15.2f} {output[i][4]: 15.2f} {output[i][5]: 15.5f} {output[i][6]: 15.2f} {output[i][7]: 15.3f} {output[i][8]: 15.3f} {output[i][9]: 15.3f} {output[i][10]: 15.3f} {output[i][11]: 15.3f} {output[i][12]: 15.3f} {output[i][13]: 15.3f} {output[i][14]: 15.3f} {output[i][15]: 15.3f} {output[i][16]: 15.3f} {output[i][17]: 15.3f} {output[i][18]: 15.3f} {output[i][19]: 15.3f} {output[i][20]: 15.3f} {output[i][21]: 15.3f} {output[i][22]: 15.3f} {output[i][23]: 15.3f}\n")

   # with open(f"{filename}_5_1.dat", "w") as f:
   #   # f.write(
   #   #    f'{"x2" : >15} {"theta" : >15} {"chi^2" : >15} {"chi^2_gamma" : >15} {"delta" : >15} {"chi^2_delta" : >15} {"m_T(TeV)" : >15} {"chi^2_K" : >15} {"chi^2_K" : >15} {"chi^2_eps" : >15} {"chi^2_D" : >15} {"chi^2_Bd_re" : >15} {"chi^2_Bd_im" : >15} {"chi^2_Bs_re" : >15} {"chi^2_Bs_im" : >15} {"chi^2_Bd_decay" : >15} {"chi^2_Bs_decay" : >15} {"chi^2_K1_decay" : >15} {"chi^2_K2_decay" : >15} {"chi^2_K3_decay" : >15} {"chi^2_Vud" : >15} {"chi^2_Vus" : >15} {"chi^2_Vub" : >15}\n\n')
   #    # for i in range(N_POINTS ** 2, N_POINTS ** 2 * 2):
   #    for i in range(N_POINTS * 25, N_POINTS * 25 * 2):
   #        f.write(
   #            f"{output[i][2] / MeV: 15.7f} {output[i][3]: 15.6f} {output[i][0]: 15.2f} {output[i][4]: 15.2f} {output[i][5]: 15.5f} {output[i][6]: 15.2f} {output[i][7]: 15.3f} {output[i][8]: 15.3f} {output[i][9]: 15.3f} {output[i][10]: 15.3f} {output[i][11]: 15.3f} {output[i][12]: 15.3f} {output[i][13]: 15.3f} {output[i][14]: 15.3f} {output[i][15]: 15.3f} {output[i][16]: 15.3f} {output[i][17]: 15.3f} {output[i][18]: 15.3f} {output[i][19]: 15.3f} {output[i][20]: 15.3f} {output[i][21]: 15.3f} {output[i][22]: 15.3f} {output[i][23]: 15.3f}\n")
...
def pool_function(input):
    ...
    print(chi_square_bfv)
    D_u = abs(D_u * abs(MASS_T / D_u[2]))

    phase = np.array([np.exp(-np.angle(V[0, 0]) * 1j),
                      np.exp(-np.angle(V[0, 1]) * 1j), 1. + 0 * 1j], dtype=np.complex128)
    V = V @ np.diag(phase)
    F = V @ V.conj().T
    chi_square_K = pheno.K0K0bar(V, D_u[1], D_u[2], D_u[3], 0)
    chi_square_eps = pheno.eps_ratio(
        V, F, D_u[0], D_u[1], D_u[2], D_u[3], 0)
    chi_square_D = pheno.D0D0bar(F, 0)
    chi_square_Bd = pheno.BdBdbar(V, D_u[2], D_u[3], 0)
    chi_square_Bs = pheno.BsBsbar(V, D_u[2], D_u[3], 0)
    ...
    return np.array([chi_square_bfv, par_values_bfv, x, theta, chi_square_gamma, delta_unitarity, chi_square_first_row, D_u[3] / TeV, chi_square_K[0], chi_square_K[1], chi_square_eps, chi_square_D, chi_square_Bd[0], chi_square_Bd[1], chi_square_Bs[0], chi_square_Bs[1], chi_square_Bd_decay, chi_square_Bs_decay, chi_square_K1_decay, chi_square_K2_decay, chi_square_K3_decay, chi_square_V_ud, chi_square_V_us, chi_square_V_ub], dtype='object')
