...
#    # Force decoupled mass to be m_d, m_s or m_b
#    chi_square_ratio_db = 1
#    if abs(M_d[2, 2]) > 1e-15:
#        if MASS_D - abs(M_d[1, 1]) > 0:
#            chi_square_ratio_db = ((MASS_D - abs(M_d[1, 1])) / UPPER_SIGMA_MASS_D) ** 2
#        else:
#            chi_square_ratio_db = ((MASS_D - abs(M_d[1, 1])) / LOWER_SIGMA_MASS_D) ** 2
#
#    if abs(M_d[2, 2]) < 1e-15:
#        if MASS_D - abs(M_d[2, 0]) > 0:
#            chi_square_ratio_db = ((MASS_D - abs(M_d[2, 0])) / UPPER_SIGMA_MASS_D) ** 2
#        else:
#            chi_square_ratio_db = ((MASS_D - abs(M_d[2, 0])) / LOWER_SIGMA_MASS_D) ** 2
...
def run_Minuit(pair, par_values, m_VLQ, control):
    ...
    M_d = np.array([[ 0.00000000e+00 +0.j,          0.00000000e+00 +0.j,          9.15069873e+01 +0.j        ],
                    [ 0.00000000e+00 +0.j,          4.77047516e+00 +0.j,         -6.79329206e-01-19.02464912j],
    ...
    with open(filename, option) as f:
        ...
        for data in set_maximally_restrictive_pairs:
            ...
            for data_point in data[1]:
                ...
                f.write(info_minimum(data_point[1], data_point[2]))
                f.write(f"\n#################################\n")
                f.write("\n")
                n_points += 1
            f.write(
...
def pool_function(input_pair):
    ...
    data = []
    #n_points = 101
    #range_mass_VLQ = np.array([4.496, 4.926, 5.356, 5.872, 6.044, 6.388, 6.560, 6.732, 7.936, 8.710, 9.656, 9.742, 9.828, 9.914, 10.00])
    #range_mass_VLQ = np.array([1.4, 3.894, 6.56, 6.732, 6.818, 6.99, 7.334, 7.85, 8.108, 8.194, 8.624, 8.968, 9.312, 9.398, 10.00])
    range_mass_VLQ = np.array([1.4, 5.528, 5.700, 5.958, 6.044, 6.216, 6.474, 6.646, 6.818, 6.990, 7.076, 7.162, 7.592, 7.764, 7.85,
                               7.936, 8.194, 8.280, 8.366, 8.452, 8.538, 8.624, 8.796, 8.882, 8.968, 9.140, 9.312, 9.484, 9.57,
                               9.656, 9.828, 10.00])
    #range_mass_VLQ = np.zeros(n_points)
    #for i in range(n_points):
    #   range_mass_VLQ[i] = (1.4 + i * (10 - 1.4) / (n_points - 1)) * TeV
    ...
    for m_VLQ in range_mass_VLQ:
        ...
        chi_square = 1e20
        i = 0
        while i < 200 and not check_chi_squares_limits(par_values, m_VLQ):
            par_values = []
            chi_square = 1e20
...
#            set_maximally_restrictive_pairs_after_minuit.append(pair)
    ...
    filename = "1_up_0_down_MRT_after_MINUIT_non_unitary_1_sigma(2HDM + only physical parameters + VLQ mass varying)_m_b.txt"
