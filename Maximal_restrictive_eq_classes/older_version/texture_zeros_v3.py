...
def get_set_non_equivalent_pairs(pairs, n_u, n_d,
    ...
    while(pairs):
            ...
            for K_R_d in set_K_R_d:
                for K_L_u in set_K_L_u:
                    ...
                    for K_L_d in set_K_L_d:
                        ...
                        # print(K_L_u)
                        # print(K_L_d)
                        set_equivalent_pairs.append([K_L_u @ pairs[0][0] @ K_R_u,
                                                     K_L_d @ pairs[0][1] @ K_R_d])
        ...
        while i < length:
            k = i + 1
            while k < length:
                if np.array_equal(set_equivalent_pairs[k][0], set_equivalent_pairs[i][0]):
                    if np.array_equal(set_equivalent_pairs[k][1], set_equivalent_pairs[i][1]):
                        set_equivalent_pairs.pop(k)
                        length -= 1
                        k -= 1
                k += 1
            i += 1
        ...
        for pair in set_equivalent_pairs:
            ...
            # Eliminate from the list "pairs" the set_equivalent_pairs
            k = 0
            with open("log", "a") as f:
            ...
            k = 0
            while not (np.array_equal(pair[0], pairs[k][0])
                       and np.array_equal(pair[1], pairs[k][1])):
                k += 1
            pairs.pop(k)
