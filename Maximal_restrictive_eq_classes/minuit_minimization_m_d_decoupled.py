...
FILENAME = "best_fit_value_b_decoupled_200.txt"
...
VLQ_MASSES = np.array([1 + i / (N_POINTS_VLQ - 1) * (7 - 1) for i in range(N_POINTS_VLQ)]) * TeV
PHASES = np.array([-0.3 + BFV_PHASE_B + i / (N_POINTS_PHASE - 1) * 0.6 for i in range(N_POINTS_PHASE)])
SIGMA_MASS_VLQ = 1e-5 * TeV
...
def compute_chi_square(D_u, D_d, V, m_VLQ):
    ...
    m_VLQ_found = abs(D_u[3]) * MASS_T / abs(D_u[2])
    chi_square_m_VLQ = ((m_VLQ - m_VLQ_found) / SIGMA_MASS_VLQ)**2
...
def least_squares(M_u_0_0_re, M_u_0_0_im, M_u_0_1_re, M_u_0_1_im, M_u_0_2_re, M_u_0_2_im, M_u_0_3_re, M_u_0_3_im,
    ...
    if MASS_DECOUPLED == MASS_D:
        decoupled = 1
    if MASS_DECOUPLED == MASS_S:
        decoupled = 2
    if MASS_DECOUPLED == MASS_B:
        decoupled = 3

    # Compute observables from texture zeros
    D_u, D_d, sin_theta_12, sin_theta_13, sin_theta_23, delta, Jarlskog, V = ext.extract_obs_m_decoupled(
        M_u, M_d, decoupled)
...
def run_Minuit(pair, par_values, m_VLQ, phase, control):
...
#        M_u = np.array([[                 0.00000000e+00,                  0.00000000e+00, (2 * rng.random() - 1) * 1e+03, (2 * rng.random() - 1) * 1e+03],
#                        [                 0.00000000e+00,  (2 * rng.random() - 1) * 1e+05,                 0.00000000e+00, (2 * rng.random() - 1) * 1e+02],
#                        [ (2 * rng.random() - 1) * 1e+02,                  0.00000000e+00, (2 * rng.random() - 1) * 1e+05,  0.00000000e+00],
#                        [                 0.00000000e+00,  (2 * rng.random() - 1) * 1e+06,                 0.00000000e+00,  0.00000000e+00]])
        for i in range(4):
            for j in range(4):
                if pair[0][i, j] == 2:
                    M_u[i, j] = 10 ** (rng.random() * 6 - 6) + 10 ** (rng.random() * 6 - 6) * 1j  # 7
                elif pair[0][i, j] == 1:
                    M_u[i, j] = 10 ** (rng.random() * 6 - 6)  # 7
                else:
                    M_u[i, j] = 0
        ...
        #    for j in range(3):
        #        if pair[1][i, j] == 2:
        #            M_d[i, j] = (10 ** (rng.random() * 3 - 3) + 10 ** (rng.random() * 3 - 3) * 1j)# 6
        #        elif pair[1][i, j] == 1:
        #            M_d[i, j] = (10 ** (rng.random() * 3 - 3))# 6
        #        else:
        #            M_d[i, j] = 0
    ...
    if pair[1][2, 2] == 0:
        if MASS_DECOUPLED == MASS_D:
            M_d = np.array([[   0.         +0.j,            0.         +0.j,         4175.7358899  +0.j        ],
                            [   0.         +0.j,            93.47294976 +0.j,        abs(162.13587918-71.24972603j) * cmath.exp(phase * 1j)],
                            [   4.67       +0.j,            0.         +0.j,         0.         +0.j        ]])
        if MASS_DECOUPLED == MASS_S:
            M_d = np.array([[   0.         +0.j,            0.         +0.j,         4180.44525982 +0.j        ],
                            [   0.         +0.j,           -4.6709246  +0.j,         abs(-33.89816159-14.87595132j) * cmath.exp(phase * 1j)],
                            [  93.4        +0.j,            0.         +0.j,            0.         +0.j        ]])
        if MASS_DECOUPLED == MASS_B:
            M_d = np.array([[ 0.00000000e+00 +0.j,          0.00000000e+00 +0.j,     9.14302009e+01 +0.j        ],
                            [ 0.00000000e+00 +0.j,         -4.77038502e+00 +0.j,     abs(5.29976550e-01+19.02586236j) * cmath.exp(phase * 1j)],
                            [ 4.18000000e+03 +0.j,          0.00000000e+00 +0.j,     0.00000000e+00 +0.j        ]])
    else:
        if MASS_DECOUPLED == MASS_D:
            M_d = np.array([[   0.         +0.j,            0.         +0.j,         4175.7358899  +0.j        ],
                            [   0.         +0.j,            4.67       +0.j,            0.         +0.j        ],
                            [  93.47294976 +0.j,            0.         +0.j,          abs(162.13587918-71.24972603j) * cmath.exp(phase * 1j)]])
        if MASS_DECOUPLED == MASS_S:
            M_d = np.array([[[   0.         +0.j,            0.         +0.j,         4179.83612338 +0.j        ],
                             [   0.         +0.j,           93.3863906  +0.j,            0.         +0.j        ],
                             [  -4.670244   +0.j,            0.         +0.j,          abs(-33.89322226-14.87378373j) * cmath.exp(phase * 1j)]]])
        if MASS_DECOUPLED == MASS_B:
            M_d = np.array([[ 0.00000000e+00 +0.j,          0.00000000e+00 +0.j,          9.14274348e+01 +0.j        ],
                            [ 0.00000000e+00 +0.j,          4.18000000e+03 +0.j,          0.00000000e+00 +0.j        ],
                            [-4.77083651e+00 +0.j,          0.00000000e+00 +0.j,          abs(6.11351251e-01+19.05543303j) * cmath.exp(phase * 1j)]])
...
def info_minimum(par_values, m_VLQ, phase):
    ...
    string += f"V:\n{V}\n"
    return string, np.real(scale_up), np.real(scale_down)
    ...
    with open(filename, option) as f:
        ...
        for data in set_maximally_restrictive_pairs:
            ...
            for data_point in data[1]:
                ...
                string, scale_up, scale_down = info_minimum(data_point[1], data_point[2], data_point[3])
                f.write(f"M_u:\n{M_u * scale_up}\n")
                f.write(f"M_d:\n{M_d * scale_down}\n")
...
#
#    data = []
#    range_mass_VLQ = VLQ_MASSES
#
#    for m_VLQ in range_mass_VLQ:
#        pair = input_pair[1]
#        par_values = []
#        chi_square = 1e20
#        i = 0
...
#            par_values = []
#            chi_square = 1e20
#            chi_square_minuit, par_values_minuit = run_Minuit(pair, par_values, m_VLQ, 0)
#            while (chi_square - chi_square_minuit) / chi_square > 1e-3:
#                chi_square = chi_square_minuit
#                par_values = par_values_minuit
#                chi_square_minuit, par_values_minuit = run_Minuit(pair, par_values, m_VLQ, 1)
#            #print(chi_square_minuit)
#            #print(info_minimum(par_values_minuit, m_VLQ))
#            i += 1
#
#        if check_chi_squares_limits(par_values_minuit, m_VLQ):
#            print(pair[0])
#            print(pair[1])
#            print(f"# de iteracoes = {i}")
#            print(f"m_VLQ = {m_VLQ / TeV} TeV")
#            print(f"total_chi_square = {chi_square_minuit}")
#            print("\n")
#
#            data.append([chi_square_minuit, par_values_minuit, m_VLQ])
#
    ...
    for m_VLQ in VLQ_MASSES:
        for phase in PHASES:
            ...
            while i < N_TRIES:
                par_values = []
                chi_square = 1e20
                chi_square_minuit, par_values_minuit = run_Minuit(pair, par_values, m_VLQ, phase, 0)
                while (chi_square - chi_square_minuit) / chi_square > 1e-5:
                    chi_square = chi_square_minuit
                    par_values = par_values_minuit
                    chi_square_minuit, par_values_minuit = run_Minuit(pair, par_values, m_VLQ, phase, 1)

                if chi_square < chi_square_bfv:
                    chi_square_bfv = chi_square
                    par_values_bfv = par_values
                i += 1
            data.append([chi_square_bfv, par_values_bfv, m_VLQ, phase])
